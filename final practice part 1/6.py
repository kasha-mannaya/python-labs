...
def show(data: list[dict], param="top", num=5, separator=", ") -> None:
    ...
    if type(num) != type(5):
        raise f"TypeError: 'num' аргумент должен быть целым числом, не {str(type(num))}"
    if type(separator) != type(", "):
        raise f"TypeError: 'separator' аргумент должен быть строкой, не {str(type(separator))}"
    if len(data) < num:
        print(f"Длинна данных меньше длины числа: {len(data)} < {num}")
        num = len(data)
...
def info(data: list[dict]) -> None:
    print(f"Размер таблицы: {len(data)} x {len(data[0].keys())}")
